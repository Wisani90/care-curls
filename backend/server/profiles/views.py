...
from profiles.models import UserProfile, Activation
...
class SignUpView(GuestOnlyView, FormView):
    ...
    def form_valid(self, form):
        ...
        if settings.DISABLE_USERNAME:
            user.username = 'user_{user_id}'.format(user_id=user.id)
            user.save()
...
def signup(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            user.profile.town = form.cleaned_data.get('town', None)
            user.profile.avatar = form.cleaned_data.get('avatar', None)
            user.save()
